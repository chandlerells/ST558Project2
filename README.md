...
(`year`) and return talent composite rankings in the form of a tibble.
If multiple seasons want to be analyzed, a form of `apply` could utilize
...
specify year `numerically` during the function call.
...
particular `season` and `team`. Stats are relevant to both the offensive
and defensive side of the ball, so plenty of things to filter on. The
...
some form of the `apply` function could be used.
...
This function allows the user to analyze high level game results for a
particular `season` and `team` such as scores, teams, attendance, and
week. The required arguments, with no defaults, are `year` and `team`,
so make sure these are populated. Year in the form of an integer and
team as a character string wrapped between quotations. One thing to note
is that this function may not work for very unique school names that
have specific characters included in their name. It should work for
...
game results, some for of the `apply` function could be used.
...
`year` and `team`. The required arguments, with no defaults, are `year`
and `team`, so make sure these are populated. Year in the form of an
integer and team as a character string wrapped between quotations. One
thing to note is that this function may not work for very unique school
names that have specific characters included in their name. It should
work for schools that don’t have any symbols. One thing that was built
in was a `&` in the second name, such as `Texas A&M`. For multiple years
worth of team records, some form of the `apply` function could be used.
...
specified is `venue_name`. This is an optional argument, so if it is not
specified, all venues will be returned. If a venue name is specified,
make sure it is spelled correctly and between quotations.
...
material difference between Alabama and Michigan, the first and tenth
highest average rankings. Luckily my alma mater, `Texas A&M`, cracks the
top 10!
...
average and arrange from highest to smallest. We’ll visualize the
results through a bar graph.
